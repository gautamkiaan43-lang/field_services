...
import { User, Phone, Mail, MapPin, Wrench, FileText, Calendar, Clock, Send, CheckCircle2, ChevronRight } from 'lucide-react'
...
export default function LeadForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitted(true)
      toast.success('Thank you! Inquiry received.')
    } catch (error) {
      console.error('Lead submission error:', error)
      toast.error('Could not submit inquiry - check connection')
    } finally {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
        <div className="w-20 h-20 rounded-3xl bg-emerald-500/10 flex items-center justify-center mb-6 border border-emerald-500/20 text-emerald-400">
          <CheckCircle2 size={40} />
        </div>
        <h3 className="text-3xl font-black text-white mb-2">Inquiry Sent!</h3>
        <p className="text-slate-400 max-w-sm mx-auto leading-relaxed">
          We've received your request. Our team will review your details and propose a service schedule shortly.
        </p>
      </div>
    )
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-4">
         ...
         <GradientButton 
            ...
            className="w-full sm:w-auto px-10 py-5 rounded-2xl text-base md:text-lg font-black tracking-widest uppercase italic shadow-2xl"
          >
